...
class GameState:
    ...
    def prompt_crib(self, player):
        print("Lay away...")
        self.crib.append(self.select_card(player)) 
        print("Lay away...")
        self.crib.append(self.select_card(player))
    ...
    def select_card(self, player):
        ...
        while True:
            ...
            inpt = self.input(list(enumerate(player.hand)))
            try:
               idx = int(inpt) 
            except ValueError:
    ...
    def cut(self):
        cut_number = int(self.input(f'Player {self.player1.id}, please select a number between 5 and {len(self.deck)} to cut the deck'))
    ...
    def start(self):
        ...
        while True:
            ...
            playable_cards = list(self.filter_playable_cards(current_player.hand, the_count))
            ...
            card_played = current_player.hand.pop(int(self.input("Play a card from your hand (enter the index of the card - first card is 0)")))
            ...
            current_player.score += self.apply_score(self.played_stack, the_count)
    ...
    def apply_score(self, played_stack, count):
        ...
        if straight_queue_len == 0:
            ...
            if abs(diff) == 1:
                ...
                self.straight.clear()
                self.straight.append(played_stack[-1]) # because it could still be this card in it, just reset the rest though
        ...
        if matches_len == 0:
            ...
            if cmp.rank == played_stack[-1].rank:
                ...
                score += matches_len + matches_len**2 # a[n] = n^2 + n is [2,6,12...]
            else:
                self.matches.clear()
                self.matches.append(played_stack[-1]) # still might have the last played card to start somthing
